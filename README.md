...
AI 에이전트 친화적인 유럽 5대 리그 축구 데이터 CLI + MCP 서버
...
- **CLI** — `--manifest` 진입점을 통해 AI 에이전트가 자율적으로 데이터를 조회
- **MCP 서버** — Claude Desktop 등 MCP 클라이언트와 직접 연동 (`football-mcp`)
...
[Releases](https://github.com/gnvvoo/football-cli/releases) 페이지에서 플랫폼에 맞는 바이너리를 다운로드합니다.

| 파일명 | 플랫폼 |
| --- | --- |
| `football-cli-linux-amd64` | Linux x86-64 |
| `football-cli-darwin-amd64` | macOS Intel |
| `football-cli-darwin-arm64` | macOS Apple Silicon |
| `football-cli-windows-amd64.exe` | Windows x86-64 |
| `football-mcp-linux-amd64` | Linux x86-64 (MCP) |
| `football-mcp-darwin-amd64` | macOS Intel (MCP) |
| `football-mcp-darwin-arm64` | macOS Apple Silicon (MCP) |
| `football-mcp-windows-amd64.exe` | Windows x86-64 (MCP) |

### 소스 빌드

```bash
git clone https://github.com/gnvvoo/football-cli.git
cd football-cli
...
football-cli matches --league EPL --from 2026-04-01 --to 2026-04-30
...
선수 정보를 조회합니다. 발음 구별 부호 무시 검색을 지원합니다 (예: `Modric` → Modrić).

```bash
football-cli player-stats --player Salah
football-cli player-stats --player "Erling Haaland"
football-cli player-stats --player Modric --league LaLiga
```

| 플래그     | 필수 | 설명                                              |
| ---------- | ---- | ------------------------------------------------- |
| `--player` | ✅   | 선수 이름 (부분 검색 가능, 발음 구별 부호 무시)   |
| `--league` |      | 리그 약어 (생략 시 전체 리그에서 검색)            |
...
`football-mcp`는 [Model Context Protocol](https://modelcontextprotocol.io) 서버로, Claude Desktop 등 MCP 클라이언트에서 축구 데이터를 직접 조회할 수 있습니다.

### 제공 툴

| 툴 이름            | 설명                                         |
| ------------------ | -------------------------------------------- |
| `get_matches`      | 리그별 경기 일정 및 결과 조회                |
| `get_standings`    | 리그 순위 조회                               |
| `get_team_info`    | 팀 정보 조회 (창단연도, 경기장, 소속 리그)   |
| `get_player_stats` | 선수 정보 조회 (포지션, 나이, 국적, 소속팀)  |
| `predict_match`    | 두 팀의 경기 결과 예측                       |
...
```json
{
  "mcpServers": {
    "football": {
      "command": "C:\\path\\to\\football-mcp-windows-amd64.exe",
      "env": {
        "FOOTBALL_DATA_API_KEY": "발급받은_API_키"
      }
    }
...
├ main.go
├ mcp/
│  └ server.go         MCP 서버 (football-mcp 바이너리)
├ cmd/
