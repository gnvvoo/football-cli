...
func NewClient(timeoutMs int) (*Client, error) {
	apiKey := os.Getenv("FOOTBALL_API_KEY") // 환경변수에서 API 키를 읽어서 Client를 초기화
	// API 호출 실패시
	if apiKey == "" {
		// (*Client, error)에 (nil, Error) 반환
		return nil, fmt.Errorf("FOOTBALL_API_KEY 환경변수가 설정되지 않았습니다")
	}
...
// Get은 API-Football에 GET 요청을 보내는 공통 함수
// endpoint 예시: "/fixtures?league=39&season=2024"
...
func (c *Client) Get(endpoint string, result any) error {
	...
	// API 레벨 에러 확인
	// API-Football은 HTTP 200이어도 errors 필드에 에러를 담아 보내는 경우가 있음
